      ...
      <section className="pt-32 pb-20 bg-gradient-to-br from-teal-700 to-teal-900 relative overflow-hidden">
        ...
        <div className="relative w-full max-w-[1400px] mx-auto px-4 md:px-6 lg:px-8 xl:px-10 text-center">
          <FadeInUp>
            ...
            <p className="text-teal-200 text-lg">{doctor.qualification} · {doctor.role}</p>
          </FadeInUp>
        </div>
      </section>
      ...
      <section className="py-20 bg-white">
        <div className="w-full max-w-[1400px] mx-auto px-4 md:px-6 lg:px-8 xl:px-10">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <SlideInLeft>
              {/* Doctor card */}
              <div className="bg-gradient-to-br from-teal-50 to-white rounded-3xl p-10 border border-teal-100 text-center">
                <div className="w-40 h-40 rounded-full bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center mx-auto mb-6 shadow-xl">
                  <span className="text-7xl">👨‍⚕️</span>
                </div>
                <h2 className="text-2xl font-bold text-gray-900 mb-1">{doctor.name}</h2>
                <p className="text-teal-600 font-medium mb-1">{doctor.role}</p>
            ...
            <SlideInRight>
              <div className="flex items-center gap-2 bg-teal-50 rounded-full px-4 py-1.5 inline-flex w-fit mb-4">
                <GraduationCap className="w-4 h-4 text-teal-600" />
                <span className="text-xs font-semibold text-teal-700 uppercase tracking-wider">About the Doctor</span>
              </div>
              ...
              <div className="flex flex-wrap gap-4">
                <a
                  href={CALL_LINK}
                  className="inline-flex items-center gap-2 bg-teal-600 text-white font-semibold px-6 py-3 rounded-full hover:bg-teal-700 transition-all hover:-translate-y-0.5"
                >
                ...
                {timeline.map((item) => (
                  <StaggerItem key={item.year}>
                    <div className="flex gap-6 items-start">
                      <div className="relative">
                        <div className="w-8 h-8 rounded-full bg-teal-600 flex items-center justify-center shrink-0 z-10 relative">
                          <Award className="w-4 h-4 text-white" />
                        </div>
                      </div>
                      <div className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm flex-1">
                        <span className="text-xs font-bold text-teal-600 mb-1 block">{item.year}</span>
          ...
          <StaggerContainer className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <StaggerItem key={v.title}>
                <div className="bg-teal-50 rounded-2xl p-7 text-center">
